        ...
        if M[i] == i:
            ...
            while True:
                if visited[next_node]:
                    if next_node in node_indexes:
                        for j in range(node_indexes[next_node], len(path)):
                            all_cycles[path[j]] = True
                    break
                visited[next_node] = True
                node_indexes[next_node] = len(path)
                path.append(next_node)
                next_node = M[next_node]

    max_perms = []
    for i in range(len(M)):
        if all_cycles[i] and M[i] != i:
            max_perms.append(i)
    return set(max_perms)
